...
public class SearchsAdapter extends RecyclerView.Adapter<SearchsAdapter.ViewHolder>  {


    private List<Search_> mData;
    ...
    public SearchsAdapter(Context context, List<Search_> data) {
    ...
    public SearchsAdapter(Context context, List<Search_> data, boolean x) {
        this.mInflater = LayoutInflater.from(context);
        this.mData = data;
        mSharedPreferences = context.getSharedPreferences(contants.pref_account,Context.MODE_PRIVATE);
        id = String.valueOf(mSharedPreferences.getInt(contants.id,0));
    ...
    public SearchsAdapter(Context context, List<Search_> data, ItemClickListener mClickListener) {
        this.mInflater = LayoutInflater.from(context);
        this.mData = data;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        final Search_ mSearchresult = mData.get(position);
        holder.TitleTextView.setText(mSearchresult.getName());
        holder.PlaceText.setText(mSearchresult.getAddress());
        holder.ratingBar.setRating(Float.parseFloat(mSearchresult.getRate()));
    ...
    public void updatelist(List<Search_> newList){
        mData = newList;
    }
